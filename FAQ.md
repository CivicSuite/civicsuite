...
**Last verified:** 2026-07-14 (civicsuite-windows-local-v1.0.4)
...
**Where to download:** get `CivicSuite_1.0.4_x64_en-US.msi` from the current GitHub Latest release tag, <https://github.com/CivicSuite/civicsuite/releases/tag/civicsuite-windows-local-v1.0.4>, and verify its SHA-256 before running: `4d86e7b217a145c72626c7dd38a87f73d41763f7b0458a4188e32a6ebf18ed42`.
...
The current release is **[civicsuite-windows-local-v1.0.4](https://github.com/CivicSuite/civicsuite/releases/tag/civicsuite-windows-local-v1.0.4)** (GitHub Latest; published 2026-07-13T21:53:01Z from current main/tag commit `e596da3`; it supersedes v1.0.3). It retains the same six-module city-core suite and adds first-run wizard UX Waves 1-3. For the full step-by-step, see [docs/installer/operator-walkthrough.md](docs/installer/operator-walkthrough.md).

This is a **GA candidate, open for public beta**: feature-complete for city-core, with inherited v1.0.2 clean-machine and accessibility-gate evidence treated as historical support only. No fresh v1.0.4 clean-machine, accessibility, or end-to-end validation execution receipt was found, so do not treat v1.0.4 as newly passed through those gates. The MSI is Authenticode code-signed via Azure Trusted Signing. Because the certificate is new, Windows SmartScreen may still show *"Windows protected your PC"* on first run — click **More info**, confirm it shows a **verified publisher** (not "Unknown Publisher"), then **Run anyway** (see [docs/troubleshooting.md](docs/troubleshooting.md#windows-smartscreen-when-you-run-the-installer)). You can download and use it now for real hands-on evaluation and early adoption. It is still a **beta** — **not yet production-, city-, or procurement-ready** — so do not run your city's system of record on it yet.

## Can my city rely on CivicSuite for live operations today?

**No.** The honest current package is the city-core beta installer profile (version labels reconciled with what actually works): CivicCore v1.2.0, CivicRecords AI v1.7.3, CivicClerk v1.0.4, CivicCode v1.0.8, CivicNotice v0.2.0, CivicAccess v0.4.0, and the suite installer. Its v1.0.2 install-lifecycle, clean-machine, accessibility-gate, first-run browser QA, green PR CI, and audit-full records remain inherited historical evidence, not fresh v1.0.4 validation proof.

That is still a bounded beta package. It is **not city-ready, not procurement-ready, not production-ready, not macOS lifecycle certified, and not a full-suite release**. CivicAccess v0.4.0 joined city-core as the sixth module on 2026-06-29 (a follow-up in-depth review qualified it for city-core, reversing the 2026-05-23 "needs work" demotion — a failed depth review); the current v1.0.4 MSI retains all six city-core modules — **the CivicAccess module code, database schema, and write-token secret are bundled, and its on-screen Accessibility workflow tab, with three local-AI tools, ships in the desktop UI** (see [docs/audits/civicaccess-citycore-deep-read-2026-06-29/FINAL-REPORT.md](docs/audits/civicaccess-citycore-deep-read-2026-06-29/FINAL-REPORT.md)). CivicZone, CivicPlan, CivicPermit, and CivicInspect remain queued Tier 2 modules on corrected version labels (versions lowered to match actual maturity).
