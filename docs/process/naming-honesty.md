...
Do not use `_live_` or `_real_wire_` in a test filename when that file
monkeypatches, stubs, or fakes the named integration boundary. A test that
...
`scripts/policy/check_test_naming_honesty.py` scans `_live_` and `_real_wire_`
test files and flags same-file monkeypatches of the named boundary. The check is
intentionally conservative: it looks for direct `monkeypatch.setattr(...)` calls
whose target shares the boundary words advertised by the filename.
