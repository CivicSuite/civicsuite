...
- Records Requests: create or submit a request, review the response deadline and basis, review the Request Timeline, Request Messages, Request Documents, and Notification Outbox, log generated notifications as sent after staff sends them, record search/citation notes, attach local source documents with hash/citation evidence, add fee line items with the fee schedule or policy basis, record a fee waiver reason, generate or type a response draft, approve, export, mark fulfilled, and close.
