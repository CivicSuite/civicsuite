# Browser QA - Local Demo Deployment Profile Link

Date: 2026-04-28T05:24:09.248Z
Target: file:///C:/Users/scott/OneDrive/Desktop/Claude/CivicSuite/docs/index.html

## Viewports
- desktop: 1440x1100, screenshot docs/browser-qa-local-demo-profile-desktop.png, body text length 18496
- mobile: 390x1000, screenshot docs/browser-qa-local-demo-profile-mobile.png, body text length 18496

## Assertions
- Local demo profile copy visible: true
- Four-service stack names visible: true
- Evaluation-vs-production boundary visible: true
- Link href present: true
- Console errors: 0

## Console Errors
- None
