...
As of 2026-05-20, the clerk-core installer evidence may include optional installed-stack workflow proof for CivicRecords AI request/search-surface/review/response handling and CivicClerk agenda/packet/minutes/vote/notice/archive handling. That proof is starter-profile lifecycle evidence only; it does not supersede per-module release gates, does not prove live cross-module records exchange, and does not create a public-use or city-ready claim. `installer-clerk-core-v0.1.0-beta.4` is the current published unsigned OSS beta outside-test release after release-lockstep, suite verification, installer verification, docs verification, and main CI passed. Promotion beyond outside-test beta is blocked by `docs/installer/starter-set-public-use-readiness-gate.md`, which is currently RED.
