...
|-----------------|--------------------------------|-----------------|------------|----------------------------|---------------|-------|
| civiccore       | CivicSuite/civiccore           | 0.19.0          | 2026-05-02 | n/a                        | 2026-05-02    | Shared platform release line now includes reusable vendor-delta planning and mock-city vendor, municipal IdP, and backup-retention contracts in addition to live-sync retry/circuit primitives, persisted audit-log hash/verification helpers, connector import helpers, browser-evidence verification helpers, shared search/access helpers, trusted-header config loading, and proxy-source enforcement helpers. |
| civicrecords-ai | CivicSuite/civicrecords-ai     | 1.4.6           | 2026-05-02 | `==0.19.0`                 | 2026-05-02    | Published records-ai release consumes the published `civiccore v0.19.0` wheel for shared vendor-delta and mock-city contracts, live-sync retry/circuit primitives, persisted audit-log hashing and verification, shared search, onboarding, connector-security, and ingest contracts. |
| civicclerk      | CivicSuite/civicclerk          | 0.1.17          | 2026-05-02 | `==0.19.0`                 | 2026-05-02    | Productizing clerk release consumes the published `civiccore v0.19.0` wheel and adapts the shared CivicCore vendor-delta and mock-city contracts while retaining CivicClerk-specific OIDC validation. It includes the React staff/public product rehearsal, Docker Compose seeded demo, OIDC browser-session foundation, Docker/PostgreSQL backup/restore rehearsal, vendor-network live sync, reusable mock municipal IdP and backup-retention contract suites, scheduled local connector import sync, installer source packaging, and enterprise signing readiness. |
...
| civiccode       | CivicSuite/civiccode           | 0.1.4           | 2026-05-02 | `==0.19.0`                 | 2026-05-02    | Active municipal-code productization release: source-registry persistence, staff-header-protected source registry operations, and the staff source registry workspace are published while the module consumes the published `civiccore v0.19.0` wheel. |
...
|------------|-----------|------------------------|--------|----------|
| 2026-05-02 | 0.19.0    | civiccode 0.1.4        | green  | civiccode PR #25 merged at 383a7f1; manual status fix PR #26 merged at 9169116; v0.1.4 docs-truth patch PR #27 merged at d56679e; `bash scripts/verify-release.sh` passed with 112 tests, docs gate, placeholder import gate, Ruff, and build artifact checks; main CI run 25252413211 passed; GitHub release `v0.1.4` published wheel, sdist, and SHA256SUMS assets |
