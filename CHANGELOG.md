...
- **2026-06-05.** fix(installer): route the city-core Windows 0.1.2 customer artifact through the Stage 3A bare-metal progress wrapper, regenerate artifacts and checksums, require tester re-gate 019 to run the artifact with real `Get-HostFacts`, make Stage3 handoff failures write terminal failed bootstrap result JSON, and add bounded Docker Desktop compose-build retry evidence with reduced Compose parallelism. Tester result 017 remains the prior green repo-local/injected-facts proof, tester result 018 records the red Docker/stale-result/artifact-path blocker, and tester result 021 passes the regenerated customer artifact from Stage0 through Stage4 with `generation_source=ollama`, `generation_model=gemma4:e4b`, and the launcher serving. No merge, tag, status promotion, or public-use/procurement/production/full-suite claim is made.
