...
- **2026-06-05.** fix(installer): route the city-core Windows 0.1.2 customer artifact through the Stage 3A bare-metal progress wrapper, regenerate artifacts and checksums, require tester re-gate 019 to run the artifact with real `Get-HostFacts`, make Stage3 handoff failures write terminal failed bootstrap result JSON, add bounded Docker Desktop compose-build retry evidence with reduced Compose parallelism, and regenerate again at `a53bad3` for phase-aware failure guidance. Tester result 017 remains the prior green repo-local/injected-facts proof, tester result 018 records the red Docker/stale-result/artifact-path blocker, tester result 021 passes the prior regenerated customer artifact from Stage0 through Stage4 with `generation_source=ollama`, `generation_model=gemma4:e4b`, and the launcher serving, and tester directive 022 requests the `a53bad3` artifact refresh re-gate. No merge, tag, status promotion, or public-use/procurement/production/full-suite claim is made.
- **2026-06-02.** feat(installer): add the Stage 3A Windows bare-metal installer scaffold on branch `stage-3a-baremetal-windows`, including a Docker Desktop spike, self-terminating WSL2 reboot-resume handling, independent Stage4 lifecycle evidence checks for `generation_source=ollama` and `generation_model=gemma4:e4b`, and a clerk-usable progress wrapper. This records scaffold and plan-mode evidence only; the bare Win11 Pro/Enterprise VM live gate remains pending and no merge, tag, status promotion, or public-use/procurement/production claim is made.
- **2026-06-01.** fix(installer): continue Stage 2 live installer audit closure at head `c87aa3e` by making the city-core workflow proof require model-generated Records AI response letters, failing hard on Ollama model-load errors, binding existing-stack verification to install provenance, hardening streamed lifecycle process exit handling, raising the local-AI readiness RAM floor to 12 GB for `gemma4:e4b`, pinning CivicRecords AI source to `cddc4d2be856badfbc7c6bdd26917a34ef535677`, and raising the bounded CPU LLM timeout to 120s with Ollama keep-alive 30m for clean-VM response-letter proof. No merge, tag, status promotion, or public-use/procurement/production claim is made by this entry.
- **2026-06-01.** fix(installer): make the city-core live installer warm Records AI Ollama first, before API/frontend stack startup, then require `gemma4:e4b` to remain resident before workflow proof can draft a response letter. The regenerated city-core package artifacts now carry the warm-first runner and refreshed checksums. No merge, tag, status promotion, or public-use/procurement/production claim is made by this entry.
