...
import rewrite.
  ...
  **Subsystem**                 **Current path in CivicRecords AI repo**                                                                      **Target path in CivicCore**                                            **Public API surface**
  ...
  LLM abstraction               backend/app/llm/ollama\_client.py, backend/app/llm/model\_registry.py, backend/app/llm/context\_budget.py     civiccore/llm/\*                                                        civiccore.llm.chat(), civiccore.llm.embed(), civiccore.llm.budget\_for(model)
  ...
  Sovereignty verification      scripts/verify\_no\_egress.sh, scripts/verify\_no\_telemetry.py, tests/sovereignty/\*                         civiccore/verification/\*, scripts/verify/\*                            civiccore.verification.run\_all()
  Admin shell (UI)              frontend/src/layouts/AdminShell.tsx, frontend/src/components/StatusBadge.tsx, frontend/src/design-tokens/\*   civiccore-ui/shell/\*, civiccore-ui/tokens/\*                           import { AdminShell, StatusBadge, tokens } from \'\@civicsuite/core-ui\'
...
proven to be shared --- not what might be shared one day.
  ...
  **Stays in CivicRecords AI**                                                       **Why**
  backend/app/requests/\* (request lifecycle, state machine, queue)                  Records-module-specific workflow. No other module has a "request" of the same shape.
  backend/app/letters/\* (response letter generation, templates)                     Records-specific output format. CivicCode will have its own answer rendering; the two are not one abstraction.
  backend/app/fees/\* (fee schedules, line items, waivers)                           Records-request fee rules are ORR/FOIA-specific. CivicPermit will need fees; that is a different fee schema.
