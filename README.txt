...
The next planning lane is **CivicCode**, not CivicZone. The CivicSuite/civiccode repo now exists as a scaffold with Milestone 0 planning complete; no runtime code has shipped. CivicZone remains the first major Tier 2 land-use product, but the canonical spec identifies CivicCode as the Tier 1 municipal-code layer that should be planned before CivicZone runtime work begins. See [docs/roadmap/civiccode-next-module-plan.md](docs/roadmap/civiccode-next-module-plan.md).
