...
def copy_suite_launcher_runtime(
    ...
    module_copy = {
        ...
        "civiccontracts": ("contracts", "CivicContracts", "Review contract drafts", "Check contract record support"),
        "civicboards": ("boards", "CivicBoards", "Review board rosters and attendance", "Check board and commission support"),
    }
...
def python_service_environment(install_root: Path, module_name: str) -> dict[str, str]:
    ...
    if module_name == "civicgrants":
        data_dir = install_root / "data" / "civicgrants"
        data_dir.mkdir(parents=True, exist_ok=True)
    ...
    if module_name == "civiccontracts":
        data_dir = install_root / "data" / "civiccontracts"
        data_dir.mkdir(parents=True, exist_ok=True)
        env.setdefault("CIVICCONTRACTS_DATA_DIR", str(data_dir))
        env.setdefault("CIVICCONTRACTS_STAFF_API_KEY", "civicsuite-local-staff-key")
    if module_name == "civicboards":
        data_dir = install_root / "data" / "civicboards"
        data_dir.mkdir(parents=True, exist_ok=True)
        env.setdefault("CIVICBOARDS_DATA_DIR", str(data_dir))
        env.setdefault("CIVICBOARDS_STAFF_API_KEY", "civicsuite-local-staff-key")
    return env
...
def verify_civicgrants_integration_contracts(module_ports: dict[str, int]) -> dict[str, object]:
    base = f"http://127.0.0.1:{module_ports['api']}"
    readiness_status, readiness = get_json(f"{base}/api/v1/civicgrants/readiness", timeout_seconds=20)
    contracts_status, contracts = get_json(f"{base}/api/v1/civicgrants/integration-contracts", timeout_seconds=20)
    ...
    passed = (
        ...
        and readiness.get("schema_ready") is True
        and contracts_status == 200
        and "civicgrants.opportunity_triage.v1" in provided_contracts
        and "civicgrants.application_outline.v1" in provided_contracts
        and "civicgrants.staff_review_queue.v1" in provided_contracts
        and "civicgrants.audit_file_export.v1" in provided_contracts
        and isinstance(downstream_ready, list)
        and "civicaccess public opportunity notices" in downstream_ready
        and "civicprocure grant-funded procurement packages" in downstream_ready
        and "civicrecords-ai grant file retention" in downstream_ready
    )
    return {
        "name": "civicgrants_integration_contracts",
        "status": "passed" if passed else "failed",
        "readiness_status_code": readiness_status,
        "readiness": readiness,
        "contracts_status_code": contracts_status,
        ...
        if passed
            ...
            "Confirm CivicGrants is pinned to the local-first staff workspace source commit.",
            "Confirm /api/v1/civicgrants/readiness reports ready=true and schema_ready=true.",
            "Confirm /api/v1/civicgrants/integration-contracts includes opportunity_triage, application_outline, staff_review_queue, and audit_file_export contracts.",
        ],
    }
...
def verify_civiccontracts_integration_contracts(module_ports: dict[str, int]) -> dict[str, object]:
    base = f"http://127.0.0.1:{module_ports['api']}"
    readiness_status, readiness = get_json(f"{base}/api/v1/civiccontracts/readiness", timeout_seconds=20)
    contracts_status, contracts = get_json(f"{base}/api/v1/civiccontracts/integration-contracts", timeout_seconds=20)
    contract_items = contracts.get("contracts", contracts.get("provides", []))
    provided_contracts = {
        str(item.get("name") or item.get("contract"))
        for item in contract_items
    ...
    passed = (
        ...
        and readiness.get("schema_ready") is True
        and contracts_status == 200
        and "civiccontracts.contract_draft.v1" in provided_contracts
        and "civiccontracts.staff_review_queue.v1" in provided_contracts
        and "civiccontracts.procurement_handoff.v1" in provided_contracts
        and "civiccontracts.records_export.v1" in provided_contracts
        and isinstance(downstream_ready, list)
        and "civicprocure award packet contract routing" in downstream_ready
        and "civicclerk agenda contract approvals" in downstream_ready
        and "civicrecords-ai contract file retention" in downstream_ready
        and "civiclegal counsel review queue" in downstream_ready
    )
    return {
        "name": "civiccontracts_integration_contracts",
        "status": "passed" if passed else "failed",
        "readiness_status_code": readiness_status,
        "readiness": readiness,
        "contracts_status_code": contracts_status,
        ...
        if passed
            ...
            "Confirm CivicContracts is pinned to the local-first staff workspace source commit.",
            "Confirm /api/v1/civiccontracts/readiness reports ready=true and schema_ready=true.",
            "Confirm /api/v1/civiccontracts/integration-contracts includes contract_draft, staff_review_queue, procurement_handoff, and records_export contracts.",
        ],
    }


def verify_civicboards_integration_contracts(module_ports: dict[str, int]) -> dict[str, object]:
    base = f"http://127.0.0.1:{module_ports['api']}"
    readiness_status, readiness = get_json(f"{base}/api/v1/civicboards/readiness", timeout_seconds=20)
    contracts_status, contracts = get_json(f"{base}/api/v1/civicboards/integration-contracts", timeout_seconds=20)
    contract_items = contracts.get("contracts", contracts.get("provides", []))
    provided_contracts = {
        str(item.get("name") or item.get("contract"))
        for item in contract_items
    ...
    passed = (
        ...
        and readiness.get("schema_ready") is True
        and contracts_status == 200
        and "civicboards.board_roster.v1" in provided_contracts
        and "civicboards.staff_review_queue.v1" in provided_contracts
        and "civicboards.notice_packet.v1" in provided_contracts
        and "civicboards.records_export.v1" in provided_contracts
        and isinstance(downstream_ready, list)
        and "civicclerk agenda board appointments" in downstream_ready
        and "civicnotice public vacancy notices" in downstream_ready
        and "civicrecords-ai board file retention" in downstream_ready
        and "civiclegal eligibility review" in downstream_ready
    )
    return {
        "name": "civicboards_integration_contracts",
        "status": "passed" if passed else "failed",
        "readiness_status_code": readiness_status,
        "readiness": readiness,
        "contracts_status_code": contracts_status,
        ...
        if passed
            ...
            "Confirm CivicBoards is pinned to the local-first staff workspace source commit.",
            "Confirm /api/v1/civicboards/readiness reports ready=true and schema_ready=true.",
            "Confirm /api/v1/civicboards/integration-contracts includes board_roster, staff_review_queue, notice_packet, and records_export contracts.",
        ],
    }
...
def verify(
    ...
    *,
    ...
    for module_name in PYTHON_SERVICE_MODULES:
        ...
        if module_name == "civiccontracts":
            checks.append(verify_civiccontracts_integration_contracts(module_ports))  # type: ignore[arg-type]
        if module_name == "civicboards":
            checks.append(verify_civicboards_integration_contracts(module_ports))  # type: ignore[arg-type]
