# Tester Directive 062 - CivicBoards retry after Python service BLAS thread cap

## Goal

Retry the CivicBoards standalone and suite integration gate after builder fixed the installer service-start failure reported in `TESTER-RESULT-061.md`.
...
- Fix under test: installed Python services now default BLAS/threaded math environment caps to `1` for `OPENBLAS_NUM_THREADS`, `OMP_NUM_THREADS`, `MKL_NUM_THREADS`, `NUMEXPR_NUM_THREADS`, and `VECLIB_MAXIMUM_THREADS` unless an operator explicitly overrides them.
...
- Prior result to read: `test-comms/TESTER-RESULT-061.md`
- Expected result file: `test-comms/TESTER-RESULT-062.md`
...
3. Confirm the checked-out commit is at or after `356a11a8fc0ad8ef3bae691f1c50b881a2dc26bf`.
4. Read `test-comms/TESTER-RESULT-061.md`.
5. Confirm `installer/modules.json` declares CivicBoards source commit `cdc6bf1b2e8012151d3767e04cd0e378638798c9`.
6. Confirm the installer source sets default Python service caps for:
   - `OPENBLAS_NUM_THREADS=1`
   - `OMP_NUM_THREADS=1`
   - `MKL_NUM_THREADS=1`
   - `NUMEXPR_NUM_THREADS=1`
   - `VECLIB_MAXIMUM_THREADS=1`
7. Confirm the installer source still includes `PYTHON_SERVICE_VENV_RETRIES`, `python_service_venv_is_transient_failure`, and `run_python_service_create_venv`.
8. Confirm the installer source still includes the `MemoryError` transient marker and retry command evidence for `--no-cache-dir`.
9. Do not edit source, generated artifacts, `installer/modules.json`, docs outside `test-comms`, or tests.
10. Run the standard clean-stack teardown from `test-comms/README.md`. Do not reboot the host.
11. Record host facts, Docker/Ollama state, `ollama ps`, ports `11435`, `18082`, and `23865`, stale `python`/`uvicorn`/`llama-server`/`ollama_llama_server` state, and available physical memory before readiness.
12. Verify the non-mutating proven-suite plan for the selected twelve-module suite.
13. Run repo-local readiness with isolated host-Ollama port `11435`, using the same module list and host-Ollama approach as `TESTER-DIRECTIVE-061`.
14. If readiness fails, stop and report the exact readiness blocker with memory/process diagnostics and the full `host_ollama_model_load` attempts array.
15. If readiness passes, run install with the same run isolation pattern.
16. If CivicBoards service start fails again, include the full `python_service_start` lifecycle entry and the complete CivicBoards service log tail.
...
19. Write `test-comms/TESTER-RESULT-062.md` and push it to `stage-3a-baremetal-windows`.

## Required CivicBoards evidence

`TESTER-RESULT-062.md` must include:

- exact branch head tested,
- confirmation `TESTER-RESULT-061.md` was read,
...
Push only `test-comms/TESTER-RESULT-062.md`. No source edits during the test run. No merge, tag, status promotion, or `modules.json` changes. Never touch any OneDrive path.
