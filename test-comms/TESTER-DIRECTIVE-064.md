# Tester Directive 064 - Windows Local MSI clean-machine city-core beta gate

## Goal

Run the first clean-machine Windows Local MSI gate for the CivicSuite city-core beta package.

This is a special one-off directive in the existing `stage-3a-baremetal-windows`
repo test channel. For this directive only, do not run the old Docker/baremetal
standing procedure from `test-comms/README.md`. That procedure remains historical
context for earlier stage-3a tests. This directive tests the Windows desktop MSI
artifact produced by PR #192.

The end-user product path under test is:

- Windows desktop application using Tauri/WebView2.
- CivicCore installed as the required shared platform.
- CivicRecords AI, CivicClerk, and CivicCode installed as the city-core module set.
- No Docker prerequisite.
- No WSL prerequisite.
- No terminal prerequisite for normal clerk use.
- Local data, local services, local file storage, local backup/restore, and local model readiness on the city's workstation.

## Required branch and artifact truth

- Repo test channel: `CivicSuite/civicsuite`
- Test channel branch: `stage-3a-baremetal-windows`
- Prior result to read for channel continuity: `test-comms/TESTER-RESULT-063.md`
- Expected result file: `test-comms/TESTER-RESULT-064.md`
- PR under test: `CivicSuite/civicsuite#192`
- Required PR head under test: `a8c6715d8434160c8ade722d9459f2247fb7369d`
- Expected GitHub Actions workflow run: `27522471421`
...
- Expected artifact ZIP SHA-256: `103ad65a27fb243b55cf9cb8043070bd2d6253e9d40c12823dbd2ecf788859a5`
- Expected MSI filename: `CivicSuite_0.1.0_x64_en-US.msi`
- Expected MSI bytes: `1639690816`
- Expected MSI SHA-256: `85b51b6cfe8f4713491261ca8bef070db5a7341419ad2f367670e177cbb11ce5`
- Expected evidence file inside artifact: `CivicSuite-msi-evidence.txt`

If PR #192 has a newer head before you begin the install, stop and write
`TESTER-RESULT-064.md` with verdict `BLOCKED - superseded artifact`, including
the newer head SHA and workflow run if visible. Do not test a superseded MSI.

If the expected MSI artifact is not available, stop and write
`TESTER-RESULT-064.md` with verdict `BLOCKED - MSI artifact unavailable`,
including the workflow run status and conclusion.

## Required artifact evidence

Before install, download the `civicsuite-windows-local-msi` artifact from the
required workflow run and record:
...
- MSI byte size,
- MSI SHA-256,
- full contents of `CivicSuite-msi-evidence.txt`,
- confirmation `NoDockerPrerequisite=true`,
- confirmation `NoWslPrerequisite=true`,
- confirmation `InstallerBundle=msi`,
- confirmation `UnsignedBetaNoticeSurface=msi-license-file`,
- confirmation `RuntimePayload=desktop/runtime/payload`.

The SHA-256 you compute locally must match the SHA-256 in the evidence file.
If it does not, stop and report `FAIL - artifact checksum mismatch`.

The builder pre-verified the artifact before publishing this directive:
...
- Read `test-comms/TESTER-RESULT-063.md` for channel continuity.
- Confirm no source, generated artifact, module manifest, or docs outside `test-comms` are edited.
- Preserve any useful prior logs outside OneDrive paths, then uninstall any prior CivicSuite desktop install.
- Remove leftover CivicSuite application data only after preserving evidence needed for diagnosis.
- Do not install Docker Desktop.
- Do not enable or require WSL.
- Do not use Docker, WSL, or repo-local bootstrap scripts as part of the clerk install path.
- Record Windows edition/build, CPU, RAM, disk free space, current user integrity/admin status, and whether WebView2 is already present.

The clerk path may use Windows installer UI, the installed CivicSuite desktop app,
and normal Windows Settings. Terminal use is allowed only for tester evidence
capture, artifact download, log collection, and result writing.
...
Defender SmartScreen appears, verify the UX matches the documented beta guidance:

- the installer is unsigned beta software,
- staff are told to choose "More info",
- staff are told to choose "Run anyway",
- staff are told to continue only when the file came from the trusted CivicSuite test source.

After install, launch CivicSuite and complete first-run setup using only the app UI:
...
- Missing model, partial download, failed download, needs verification, needs runtime, needs load, and ready states are plain-English and distinct when applicable.
- If the model cannot be fully downloaded, checksum-verified, loaded, and registered during the clean-machine run, mark this gate failed or blocked with exact cause. Do not mark a skipped model as pass.
...
- create a meeting body with statutory basis, cadence, notice, quorum, and member roster,
...
- calculate notice deadline with lead days, day type, time zone, and holiday review warning,
...
- confirm public archive excludes closed-session files, staff-only notes, staff-only citations, intake queue internals, and local workstation paths.
...
- confirm public status excludes staff notes, search details, exemption reasoning, local paths, and internal audit details.
...
- verify filename/hash evidence is retained without publishing workstation paths,
- record codifier/sync state,
- generate or type staff guidance,
- approve guidance,
- publish source,
- answer a code question with citations,
- create a Clerk-to-Code or Code-to-Clerk handoff tied to the meeting/adoption workflow.
...
- verify Clerk adopted ordinance/resolution evidence appears in CivicCode handoff state,
- verify Records can cite Clerk packet/minutes or Code source evidence when appropriate,
...
- close CivicSuite,
- reopen CivicSuite,
- verify city profile, users, module data, model readiness, and health state persist,
- reboot Windows,
- reopen CivicSuite,
...
- verify the support bundle does not copy city records, uploaded documents, backup contents, or local secrets,
...
- verify the app can restore from the final backup when the backup folder is available,
- verify restored city data, users, and module workflow records are present after reinstall.

If restore-from-final-backup is not exposed or fails, report exact blocker.

## Required result file format

Write `test-comms/TESTER-RESULT-064.md` with:

- final verdict: PASS, FAIL, or BLOCKED,
- tested branch and commit for the repo channel,
- PR #192 head SHA tested,
- workflow run/job/artifact evidence,
...
- evidence paths for screenshots, logs, manifests, support bundle summaries, and exported packages,
...
non-durable requirement is a FAIL unless it is clearly an external artifact or
host blocker, in which case use BLOCKED with evidence.

## Constraints

Push only `test-comms/TESTER-RESULT-064.md` to `stage-3a-baremetal-windows`.
