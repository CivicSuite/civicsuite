...
- **On the TESTER machine (Codex) — STANDING re-run procedure (no new directive needed each cycle):** `check repo` =
  ...
  2. **Clear the prior Docker stack state:** run `powershell -ExecutionPolicy Bypass -File installer\baremetal\windows\civicsuite-stack-teardown.ps1`. This removes the city-core Docker containers/volumes/networks (so Postgres re-inits with fresh credentials and you don't trip on stale state) while KEEPING Docker/WSL2/Ollama/the pulled model/Python — no re-provisioning. Skip only on a genuinely fresh box that has never run the stack.
  3. Confirm `(Get-CimInstance Win32_ComputerSystem).HypervisorPresent` is `True` and record both that value and `Win32_Processor.VirtualizationFirmwareEnabled` in the result. Do **not** inject or correct host facts for the next run; Stage0 must live-prove `Get-HostFacts`.
  4. Test the regenerated customer artifact, not the repo-local bootstrapper: run `installer\dist\CivicSuite-city-core-windows-0.1.2.cmd` from the branch checkout. The `.cmd` extracts the customer zip and launches `installer\baremetal\windows\civicsuite-baremetal-progress.ps1` from the extracted bundle. Let it self-elevate, install, and **reboot** as many times as needed. After each reboot, re-launch and `check repo` to continue.
  5. When it reaches a terminal point — the AI-letter proof, OR a blocker it can't pass — write the **next-numbered** `TESTER-RESULT-NNN.md` (diagnosis, per-stage results, the critical `generation_source`/`generation_model` check, evidence paths, honest notes) and push it. Your only acknowledgment is the pushed result file, never a summary.
