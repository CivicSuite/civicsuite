...
- **On the TESTER machine (Codex) — STANDING re-run procedure (no new directive needed each cycle):** `check repo` =
  ...
  3. Confirm `(Get-CimInstance Win32_ComputerSystem).HypervisorPresent` is `True` (VT-x is genuinely on; the installer's own check is a not-yet-fixed false-negative).
  4. Run the bootstrapper end-to-end with the corrected `-HostFactsJson` (real host facts, `virtualization_firmware_enabled=true`) so Stage0 passes; let it self-elevate, install, and **reboot** as many times as needed. After each reboot, re-launch and `check repo` to continue.
