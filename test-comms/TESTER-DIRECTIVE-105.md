# TESTER-DIRECTIVE-105

Purpose: rerun the clean-machine installed-app AI readiness gate after the product fix for TESTER-RESULT-104's Gemma download/checksum cleanup failure.
...
- Result file must be exactly `test-comms/TESTER-RESULT-105.md`.
...
- Fix summary: retries a corrupt full-size Gemma download from a clean file, treats already-removed invalid partial files as idempotent cleanup, and reports repeated checksum failure explicitly instead of surfacing a missing-file cleanup error.
- Public prerelease: https://github.com/CivicSuite/civicsuite/releases/tag/windows-local-msi-ci-1048f2a
- MSI asset: `CivicSuite_0.1.0_x64_en-US.msi`
- MSI bytes: `1645272380`
- MSI SHA-256: `1ceb650065d53f2a0ea702659b7ffb24d4be756ca13fde3300ceebc1ebafe4a7`
- Evidence asset: `CivicSuite-msi-evidence.txt`
- Evidence bytes: `578`
- Evidence SHA-256: `82bc211b5a4bde0b82b905a8d3fe2aca5492de46b5f39905bde60d113a3f4d3c`
- Workflow run: `27871321638`

Why this directive exists:

- TESTER-RESULT-104 showed the installed product reached the Local AI Model setup screen with correct pinned Gemma metadata and began a product-controlled download under CivicSuite's local model path.
- The model setup gate then failed with: `Could not remove invalid partial model download after its checksum did not match the pinned Gemma model: The system cannot find the file specified. (os error 2)`.
- After failure, the model directory was empty, no verified `.gguf` file existed, no `.part` file remained, and the AI workflows could not be reached.
- This directive must prove whether `1048f2a` lets the installed app recover from that download/checksum cleanup path and complete the bundled Ollama/Gemma readiness gate.
...
6. Prove the pinned Gemma model setup path from the installed app:
   ...
   - If a checksum failure occurs during the first attempt, retry through the installed app control and verify the product starts a clean download or resumes a valid partial without surfacing the old missing-file cleanup error.
...
8. Prove AI wiring through installed module workflows with marker `D105-AI-MODEL-MARKER-20260620`:
...
- If any step fails, `TESTER-RESULT-105.md` must say `Verdict: FAIL`.
...
- `TESTER-RESULT-105.md` may say `Verdict: PASS` only if the clean-machine installed app proves:
  ...
  - no old `Could not remove invalid partial model download ... os error 2` failure appears,
