# VMHOST-DIRECTIVE-016 - model LOAD via fire+poll (fixes 015) + real completion + backup/restore. LIVE.
...
$Result=Join-Path $VDir 'VMHOST-RESULT-016.md'; $Live=Join-Path $VDir 'VMHOST-LIVE-016.md'
$TestDir='C:\CivicSuiteCleanTest016'; $RunId='28253830442'; $RepoSlug='CivicSuite/civicsuite'
...
function Push-Live { param([string]$line)
  ...
  $hdr=@("# VMHOST-LIVE-016 - model load + completion + backup/restore (LIVE)","","Machine: $env:COMPUTERNAME   Updated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')","",'```')
  $body=$hdr+$script:logLines+@('```')
  Set-Location $Repo; git fetch origin $Branch --force 2>&1 | Out-Null; git checkout -f -B $Branch FETCH_HEAD 2>&1 | Out-Null
  Set-Content -Path $Live -Value ($body -join "`r`n") -Encoding UTF8
  git add -- $Live 2>&1 | Out-Null
  git -c user.name='vmhost-runner' -c user.email='vmhost@localhost' commit -m "vmhost: live 016 $(Stamp)" 2>&1 | Out-Null
...
function Push-Result { param([string]$body)
  ...
  git -c user.name='vmhost-runner' -c user.email='vmhost@localhost' commit -m "vmhost: result 016 model load+completion+backup" 2>&1 | Out-Null
  git push origin "HEAD:$Branch" 2>&1 | Out-Null }
if (Test-Path $Result) { $e=Get-Content $Result -Raw -ErrorAction SilentlyContinue; if ($e -match 'VERDICT') { Write-Host 'done'; exit 0 } }
Push-Live "Directive 016 started (model load fire+poll + completion + backup/restore)."
$feat=Get-WindowsOptionalFeature -Online -FeatureName 'Containers-DisposableClientVM' -ErrorAction SilentlyContinue
if (-not $feat -or $feat.State -ne 'Enabled') { Push-Result "# VMHOST-RESULT-016 - FAIL: Sandbox not enabled."; exit 1 }
...
if (-not $msi) { Push-Result "# VMHOST-RESULT-016 - FAIL: no MSI."; exit 0 }
...
Push-Result @"
# VMHOST-RESULT-016 - model load + completion + backup/restore: $v
