# Tester Directive 042 - native host-Ollama fallback full gate rerun

## Goal

Rerun the Stage 3A proven-suite clean-machine gate after builder commit `e997f508c7838c0b095a7ce66fbbad4f074e781a` added a final `native_default` host-Ollama generation probe with no synthetic `options` payload.

`TESTER-RESULT-041.md` proved the isolated `11435` path starts, `/api/tags` passes, and default-port cleanup access denial no longer blocks model attempts. It also proved every forced GPU/CPU profile failed on the available host. This directive must prove whether the actual host Ollama path Scott says runs `gemma4:e4b` succeeds when the bootstrapper lets Ollama choose its native defaults.

## Required branch truth

- Repo: `CivicSuite/civicsuite`
- Branch: `stage-3a-baremetal-windows`
- Required minimum head: `e997f508c7838c0b095a7ce66fbbad4f074e781a`
- Builder fix under test: `e997f508c7838c0b095a7ce66fbbad4f074e781a`
- Prior result to read: `test-comms/TESTER-RESULT-041.md`
- Expected result file: `test-comms/TESTER-RESULT-042.md`
...
3. Confirm the checked-out commit is at or after `e997f508c7838c0b095a7ce66fbbad4f074e781a`.
4. Read `test-comms/TESTER-RESULT-041.md`.
...
11. Inspect the readiness lifecycle report. It must include:
   ...
   - an attempt with `profile=native_default` and `options=null`
   - `host_ollama_model_load.selected_profile=native_default` if readiness passes through the new fallback
...
```powershell
python scripts\run-clerk-core-installer.py install --run-id stage3a-proven-suite-clean-machine-r18 --install-root installer\runtime\proven-suite-clean-machine-r18 --compose-project-suffix stage3a-proven-suite-clean-machine-r18 --port-offset 5000 --module civicrecords-ai --module civicclerk --module civiccode --module civiczone --module civicplan --module civicpermit --module civicaccess --module civicinspect --module civicgrants --module civicprocure --host-ollama --host-ollama-port 11435
```
...
```powershell
python scripts\run-clerk-core-installer.py verify --run-id stage3a-proven-suite-clean-machine-r18-verify --install-root installer\runtime\proven-suite-clean-machine-r18 --compose-project-suffix stage3a-proven-suite-clean-machine-r18 --port-offset 5000 --module civicrecords-ai --module civicclerk --module civiccode --module civiczone --module civicplan --module civicpermit --module civicaccess --module civicinspect --module civicgrants --module civicprocure --host-ollama --host-ollama-port 11435
```

16. If install and verify pass, inspect launcher config and live routes as in the prior proven-suite directives.
17. Write `test-comms/TESTER-RESULT-042.md` and push it to `stage-3a-baremetal-windows`.

## Required result evidence

`TESTER-RESULT-042.md` must include:

- exact branch head tested,
- confirmation `TESTER-RESULT-041.md` was read,
...
- explicit evidence for `native_default`: attempt index, `options=null`, return code, stdout/stderr, and whether it was the selected profile,
...
Push only `test-comms/TESTER-RESULT-042.md`. No source edits during the test run. No merge, tag, status promotion, or `modules.json` changes. Never touch any OneDrive path.
