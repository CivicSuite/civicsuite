# TESTER-DIRECTIVE-103

Purpose: rerun the clean-machine installed-app AI readiness gate after the product fix for TESTER-RESULT-102's chunked local AI response parsing failure.
...
- Result file must be exactly `test-comms/TESTER-RESULT-103.md`.
...
- Fix summary: decodes `Transfer-Encoding: chunked` local Ollama HTTP response bodies before parsing JSON, while preserving plain JSON response handling and empty-output detection.
- Public prerelease: https://github.com/CivicSuite/civicsuite/releases/tag/windows-local-msi-ci-4e5626d
...
- Evidence bytes: `578`
- Evidence SHA-256: `7e63b0c086e237c9ef2e43a43c70463f40ca6430caece7eddc071474712f184d`
- Workflow run: `27864517558`
...
- Commit `133874adcbbd89e4b2210faea85d62091acd7461` parsed Ollama-compatible generate response variants; TESTER-RESULT-102 then showed product-managed Ollama still returned `HTTP 200`, but the installed product tried to parse chunk-size lines such as `94` and `11` before persisting CivicRecords, CivicCode, and CivicClerk AI drafts.
- This directive must prove whether `4e5626d` decodes the real bundled Ollama/Gemma chunked response body correctly and persists usable staff-review drafts.
...
8. Prove AI wiring through installed module workflows with marker `D103-AI-MODEL-MARKER-20260620`:
...
- If any step fails, `TESTER-RESULT-103.md` must say `Verdict: FAIL`.
...
- `TESTER-RESULT-103.md` may say `Verdict: PASS` only if the clean-machine installed app proves:
