# TESTER-DIRECTIVE-102

Purpose: rerun the clean-machine installed-app AI readiness gate after the product fix for TESTER-RESULT-101's empty local AI draft failure.
...
- Result file must be exactly `test-comms/TESTER-RESULT-102.md`.
...
- Fix summary: parses local Ollama `/api/generate` response variants, including top-level `response`, chat-style `message.content`, and streamed JSON-line chunks, while preserving empty-output failure detection.
- Public prerelease: https://github.com/CivicSuite/civicsuite/releases/tag/windows-local-msi-ci-133874a
...
- Evidence bytes: `578`
- Evidence SHA-256: `c59f1d3a4ba37cf6a849a321477450d80095e1379e90a1d95eb636ac290b3b69`
- Workflow run: `27861376828`
...
- TESTER-RESULT-101 still failed because CivicRecords AI, CivicCode, and CivicClerk AI each returned `Local AI returned an empty draft.` and did not persist usable workflow output.
- This directive must prove whether `133874a` parses the real bundled Ollama/Gemma response correctly and persists usable staff-review drafts.
...
8. Prove AI wiring through installed module workflows with marker `D102-AI-MODEL-MARKER-20260620`:
...
9. Capture product-managed Ollama evidence after each AI action:
   ...
   - whether the generated text is bounded/usable rather than empty or timed out.
...
- If any step fails, `TESTER-RESULT-102.md` must say `Verdict: FAIL`.
...
- `TESTER-RESULT-102.md` may say `Verdict: PASS` only if the clean-machine installed app proves:
