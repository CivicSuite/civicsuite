...
The active city-core promotion package is CivicCore, CivicRecords AI, CivicClerk, CivicCode, and the suite installer. The current released module cars are CivicCore `v1.2.0`, CivicRecords AI `v1.7.3`, CivicClerk `v1.0.3`, and CivicCode `v1.0.8`; PR #183 records Implemented, pending first-run browser QA and audit-full evidence for the four-module non-technical Windows/Linux installer path. PR #183 is green at `8ee2c59` for verify run `26486410138`, release-lockstep-gate run `26486410136`, and installer-cleanroom run `26486410137`. The package remains bounded to unsigned vendored-source installer artifacts; macOS remains beta-level readiness only until a matching-host macOS lifecycle is proven.
...
- The suite-level `city-core` installer evidence for PR #183 records Windows and Linux one-click wrapper smoke, Guided/Manual Docker prerequisite setup paths, first-run wizard smoke, 60 GB cleanroom hygiene, local matching-host install/repair/verify/backup/restore/uninstall lifecycle evidence, and green PR CI under `C:\Users\scott\OneDrive\Desktop\Claude\CivicSuite-city-core-caboose-item1\.agent-runs\2026-05-26-city-core-non-technical-installable\`. This is Implemented, pending first-run browser QA and audit-full, not public-use readiness, city-ready status, procurement readiness, production readiness, macOS lifecycle certification, or full-suite release.
