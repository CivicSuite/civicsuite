...
The active city-core promotion package is CivicCore, CivicRecords AI, CivicClerk, CivicCode, and the suite installer. The current released module cars are CivicCore `v1.2.0`, CivicRecords AI `v1.7.3`, CivicClerk `v1.0.3`, and CivicCode `v1.0.8`; PR #183 records Implemented, pending audit-full evidence for the four-module non-technical Windows/Linux installer path. PR #183 is green at `8ee2c59` for verify run `26486410138`, release-lockstep-gate run `26486410136`, and installer-cleanroom run `26486410137`; local first-run browser QA now covers Linux `.run` and Windows `.cmd` one-click artifacts. The package remains bounded to unsigned vendored-source installer artifacts; macOS remains beta-level readiness only until a matching-host macOS lifecycle is proven.
