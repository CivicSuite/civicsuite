...
| civicaccess | v0.4.0 city-core module release | Sixth city-core module on CivicCore v1.2.0; accessibility + records-ready export; ships in the six-module Townlight Windows Local v1.0.2 release with three features on the suite's shared local AI engine. Phase D `clean_vm_dod_passed` PASS; `accessibility_passed` FAIL on v1.0.2 (the Accessibility tab was unreachable in the shipped shell and records-export wrote no artifact; both fixed on main). |
