...
Today, multiple modules are shipping at different maturity levels: CivicRecords AI is production-usable, while CivicClerk, CivicCode, CivicZone, CivicAccess, CivicPlan, CivicPermit, and CivicInspect have runtime-foundation releases. The rest are planned. We say so plainly below — no roadmap inflation, no vaporware.
...
- Seventeen additional modules across seven tiers — see the [module catalog](specs/01_catalog.md).
