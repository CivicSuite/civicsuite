...
CivicSuite is **not procurement-ready** today. The active city-core promotion package is CivicCore v1.2.0, CivicRecords AI v1.7.3, CivicClerk v1.0.3, CivicCode v1.0.8, and the suite installer. Those four module release cars exist, and the Stage 3A Windows customer-artifact bare-metal gate passed on 2026-06-05 in tester result 022 with refreshed artifact hashes, real `generation_source=ollama`, and `generation_model=gemma4:e4b` evidence. After that gate, the repo-local Stage 3A proven-suite smoke added source-pinned local install/verify evidence for CivicZone, CivicPlan, CivicPermit, CivicAccess, CivicInspect, CivicGrants, and CivicProcure alongside city-core, including live launcher wiring for all ten selected services. That local proof is not a clean-machine gate. This is not a merge, tag, status promotion, public-use readiness, city-ready status, procurement readiness, production readiness, macOS lifecycle certification, airgap readiness, or full-suite release.
...
|---|---:|---|
| City-core release cars | CivicCore plus 3 product repos | CivicCore v1.2.0 is the shared platform release. CivicRecords AI v1.7.3, CivicClerk v1.0.3, and CivicCode v1.0.8 are the city-core module cars. Stage 3A tester result 022 proves the refreshed Windows customer artifact can install from Stage0 through Stage4 with local Ollama/gemma4:e4b evidence and matching artifact hashes. PR #183 remains historical predecessor evidence; the active Stage 3A test-comms evidence is on branch `stage-3a-baremetal-windows`. |
| Source-pinned readiness modules | 7 product repos | CivicZone, CivicPlan, CivicPermit, CivicAccess, CivicInspect, CivicGrants, and CivicProcure are source-pinned on CivicCore v1.2.0 and passed repo-local proven-suite install/verify plus launcher wiring smoke. They remain clean-machine-gate pending and are not public-use, city-ready, procurement-ready, or full-suite promoted. |
| Foundation / excluded modules | 15 named product modules | The rest of the visible catalog has bounded runtime foundations or implementation specs. These are not city-ready products. `CivicRegWatch` and `CivicAPI` are planned modules with detailed specs but no runtime repos yet. The reconciled unified spec, installer metadata, and live GitHub org state now enumerate 27 product modules plus CivicCore. |
...
- **`civiczone`, `civicplan`, `civicpermit`, `civicaccess`, `civicinspect`, `civicgrants`, `civicprocure`** - source-pinned readiness modules on CivicCore v1.2.0. Stage 3A local proven-suite install/verify and launcher wiring smoke passed on 2026-06-05; clean-machine suite gate and public-use/product readiness remain pending.
...
5. Re-gate the source-pinned readiness modules on the clean machine before any broader suite promotion.

## Quick Start

**Suite installer (current):** Clerk-Core has a published bounded starter installer release, city-core is the active beta-ready truth-reconciled profile that adds CivicCode to CivicCore + CivicRecords AI + CivicClerk, and `proven-suite` is the local integration profile for city-core plus the seven source-pinned readiness modules. The Stage 3A Windows customer-artifact gate is green on branch `stage-3a-baremetal-windows` as of tester result 022. The repo-local proven-suite smoke passed install/verify and launcher wiring on 2026-06-05, and tester result 028 proves low-memory Windows hosts now fail cleanly before install. The separate proven-suite clean-machine suite gate remains blocked until a qualifying Windows host is available with at least 24 GB host RAM and Docker Desktop / WSL2 reporting at least 12 GB memory. The installer treats Ollama model-load failures as install failures, requires Records AI response-letter proof to identify `generation_source=ollama` and `generation_model=gemma4:e4b`, writes install provenance for existing-stack verification, and streams lifecycle logs to `installer/reports/<run-id>/launcher-output/`. CivicSuite's core runtime path is Linux/container-first; Windows is a wrapper around the same containerized core; macOS remains beta/archive/readiness only.
