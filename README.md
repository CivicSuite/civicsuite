...
- **`civicclerk v0.1.16`** is the clear second-product candidate. It ships the React staff workspace and public portal, all four MVP meeting-workflow surfaces, Docker Compose product rehearsal, seeded Brookfield demo data, OIDC browser-session foundations, backup/restore rehearsal, vendor-network live sync with shared CivicCore retry/circuit primitives, reusable mock municipal IdP and backup-retention contract suites, scheduled local connector import sync, installer source packaging, enterprise signing readiness, and shared `civiccore` v0.18.1 reuse. It remains in the productizing tier until production deployment hardening is complete. Repo: <https://github.com/CivicSuite/civicclerk>
