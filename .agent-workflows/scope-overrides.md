...
## 2026-06-13 - Windows Local 1.0 desktop implementation

- Advances target: `CivicSuite Windows Local 1.0 city-core desktop completion`
- Override reason:

  > Scott explicitly authorized implementation of the Windows Local 1.0 plan in
  > chat on 2026-06-13. This supersedes the stale CivicInspect queue and the
  > older Docker/browser city-core installer track. The override is limited to
  > the Windows desktop app, portable-native local runtime, city-core module
  > completion, local model path, non-technical installer, module package
  > foundation, and the verification gates named in the plan. It does not
  > authorize destructive release operations, code signing purchases, tag
  > rewrites, production secrets, or promotion of future modules without their
  > own proof gates.
